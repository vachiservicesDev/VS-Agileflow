    ...
    if (ws) {
      ws.on('participant-joined', (data) => {
        updateRoomState(prev => ({
          ...prev,
          participants: [...prev.participants, data.participant as Participant]
        }));
      });
      ...
      ws.on('vote-cast', (data) => {
        updateRoomState(prev => ({
          ...
          votes: {
            ...prev.votes,
            [data.participantId as string]: data.vote as RoomState['votes']
          }
        }));
      });
      ...
      ws.on('note-added', (data) => {
        updateRoomState(prev => ({
          ...prev,
          notes: [...(prev.notes || []), data.note as RoomState['notes']]
        }));
      });
